...
pub struct GraphicsContext {
    ...
    pub config: Arc<AtomicRefCell<SurfaceConfiguration>>,
...
impl GraphicsContext {
    pub fn resume(&mut self, window: Arc<Window>) {
        ...
        let mut config = self.config.as_ref().borrow_mut();
...
impl WgpuRenderer {
    pub async fn new(
        ...
        let mut main_camera = Camera2D::new(BaseCamera::new(vec3(0.0, 0.0, -1.), 0.01, 10000.0), 540.0);
        ...
        let camera_bind_group_layout = context.device.create_bind_group_layout(&BindGroupLayoutDescriptor {
        ...
        let hdr_bind_group_layout = 
            create_hdr_bind_group_layout(&context.device);
        let tonemapping_pipeline = 
            create_tonemapping_pipeline(&context.device, &context.config.borrow(), &hdr_bind_group_layout);
        let hdr_texture = 
            create_hdr_texture(&context.device, &context.config.borrow());
        let hdr_bind_group = 
    ...
    pub(crate) fn resize(&mut self, mut new_size: PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            if let Some(surface) = &self.context.surface.as_mut() {
                let mut config = 
                self.context.config.borrow_mut();
